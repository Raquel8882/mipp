...
export default function ResponderOmisionPage(){
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try{
      const res = await fetch(`/api/omisionmarca/${id}/responder`, {
        method: 'POST',
  ...
  return (
    <div style={{ maxWidth: 800, margin: '2rem auto', padding: 24 }}>
      <nav style={{ marginBottom: 12 }}>
        <Link href={`/omisionmarca/${id}`}>← Volver a la omisión</Link>
      </nav>
      <h2>Responder omisión #{id}</h2>
        ...
        <>
          <div style={{ background:'#f7f7f7', padding:12, borderRadius:6, marginBottom:16 }}>
            ...
            <p>
              ...
              responde a la omisión de marca; bajo la resolución de:
            </p>
          </div>
