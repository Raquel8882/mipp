-- Time control for testing: single-row table to offset perceived CR time
create table if not exists public.time_control (
  id boolean primary key default true,
  offset_minutes integer not null default 0,
  updated_at timestamptz not null default now()
);

insert into public.time_control(id) values(true)
on conflict (id) do nothing;

-- Function: get_now_cr() returns CR local time (timestamp) with offset applied
create or replace function public.get_now_cr()
returns timestamp as $$
declare
  off int := 0;
begin
  select offset_minutes into off from public.time_control limit 1;
  return (now() at time zone 'America/Costa_Rica') + make_interval(mins => coalesce(off, 0));
end;
$$ language plpgsql stable;

-- Function: get_today_cr() returns CR date with offset applied
create or replace function public.get_today_cr()
returns date as $$
begin
  return (public.get_now_cr())::date;
end;
$$ language plpgsql stable;

-- Update justificaciones trigger to use time control functions
